...
export default function AccountsPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      const balanceDelta = form.transactionType === "income" ? amount : -amount;
      const bucketDelta = form.transactionType === "income" ? amount : -amount;
      ...
      if (selectedBucket && form.moneyBucketId) {
        const bucketUpdate = await supabase
          .from("money_buckets")
          .update({ current_balance: Number(selectedBucket.current_balance) + bucketDelta })
